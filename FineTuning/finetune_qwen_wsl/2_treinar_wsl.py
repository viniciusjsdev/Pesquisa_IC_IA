#!/usr/bin/env python3
"""
Treinamento Qwen2.5-7B com LoRA/QLoRA (WSL) - versao parametrizada para multi-fine-tuning.
- Config via YAML
- Paths sem hardcode
- Artefatos em results/experiments/<protocolo>/<experimento>/<run>/
...
def resolve_runtime_config(args: argparse.Namespace) -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Path], Dict[str, Any]]:
    ...
    variables = {
        'repo_root': str(root),
        'hf_cache_dir': nested_get(paths_cfg, 'hf.cache_dir', '/mnt/e/Qwen'),
    }
...
def main() -> int:
    ...
    print('[INFO] output legado em finetune_qwen_wsl/output permanece preservado; novos runs seguem output.root_runs_dir.')
