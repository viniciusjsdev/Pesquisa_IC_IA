...
options = ["A", "B", "C", "D", "E", "F"]
ids_to_remove = [] # remove data points that have been used as demonstration data
...
def get_raw_data(raw_data_dir, data_name, cal_ratio):
    ...
    # split by asset
    all_assets = np.unique([item['asset_name'] for item in raw_data])
    train_assets, test_assets = train_test_split(all_assets, train_size=cal_ratio, random_state=42)
    cal_raw_data = [item for item in raw_data if item['asset_name'] in train_assets]
    test_raw_data = [item for item in raw_data if item['asset_name'] in test_assets]
...
def get_logits_data(model_name, data_name, cal_raw_data, test_raw_data, 
    ...
    for m in prompt_methods:
        for fs in icl_methods:
            ...
            # split logits by asset
            cal_ids = [item['id'] for item in cal_raw_data]
            test_ids = [item['id'] for item in test_raw_data]
            cal_logits_data = [item_logits for item_logits in logits_data if item_logits['id'] in cal_ids]
            test_logits_data = [item_logits for item_logits in logits_data if item_logits['id'] in test_ids]
            # variable number of options
            for i, item in enumerate(cal_logits_data):
                n_choices = len(cal_raw_data[i]['choices'])
                item['logits_options'] = cal_logits_data[i]['logits_options'][:n_choices]
            for i, item in enumerate(test_logits_data):
                n_choices = len(test_raw_data[i]['choices'])
                item['logits_options'] = test_logits_data[i]['logits_options'][:n_choices]
